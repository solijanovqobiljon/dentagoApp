...
import { useCart } from "../CartContext"; // 1. Hookni import qilamiz
...
function Boshsaxifa() {
  const { addToCart } = useCart(); // 2. Funksiyani olamiz
    ...
    <div className="min-h-screen bg-white relative">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-5 gap-4 md:gap-8">
            {categories.map(({ id, label, Icon, path }) => (
              <Link key={id} to={path} onClick={() => setActiveTab(id)} className="flex flex-col items-center gap-4">
                <div className={`w-16 h-16 md:w-20 md:h-20 rounded-full flex border-2 items-center justify-center transition-all ${activeTab === id ? 'bg-cyan-500 border-cyan-500 text-white shadow-lg' : 'bg-white border-cyan-500 text-cyan-500'}`}><Icon className="text-3xl md:text-4xl" /></div>
        ...
        <div className="flex items-center justify-between px-4 md:px-8">
          ...
          <div onClick={() => setIsModalOpen(true)} className="px-6 py-2 font-medium text-[16px] bg-[#BDF3FF] rounded-[10px] cursor-pointer text-black hover:bg-[#a2e9f7] transition-colors">Barchasi</div>
        </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-white z-[100] overflow-y-auto animate-in slide-in-from-bottom duration-300">
          <div className="sticky top-0 bg-white p-4 flex items-center gap-4 border-b border-gray-100 z-10">
            <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-gray-100 rounded-full transition-colors"><ArrowLeft size={28} className="text-black" /></button>
...
// 3. ProductCard komponentiga addToCart funksiyasini qo'shamiz
function ProductCard({ product, navigate, addToCart }) {
  const handleAddToCart = (e) => {
    e.stopPropagation(); // Card navigate bo'lib ketmasligi uchun
    
    // Savat tushunadigan formatga o'tkazamiz
    const productData = {
      id: product.id,
      nomi: product.name,
      narxi: parseInt(product.price.replace(/\s/g, "")), // "260 000" -> 260000
      image: product.img,
      quantity: 1
    };
  ...
  return (
    ...
    >
      ...
      <div className="mt-auto">
        ...
        <button 
          onClick={handleAddToCart} // 4. Yangi funksiyani uladik
